...
public class DbcpPoolManager {
	...
	private static Hashtable<String, DbcpPool> dbcpPool = new Hashtable<>();// 连接池
	private Map<String, HashMap<String, HashMap<String, Config>>> drivers = new HashMap<>();// 驱动信息
	private long time = 0;
			...
			if (keyArray.length > 3 && valueArray.length > 0) {
				...
				for (int i = 0; i < valueArray.length; i++) {
					...
					if (resuleArray.length == 2) {
						...
						if (resuleArray[0].equals("drive"))
							config.setDrive(resuleArray[1]);
					}
				}
